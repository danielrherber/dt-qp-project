...
% A1 = rand(2,2); A2 = rand(2,2); auxdata.t = (0:4)';
% matrices = {'prod',A1,A2};
% A = DTQP_tmultiprod(matrices,auxdata);
% A(:,:,1) =
%     0.3754    0.2877
%     0.3754    0.2877
%     0.3754    0.2877
...
%--------------------------------------------------------------------------
function A = DTQP_tmultiprod(matrices,auxdata,varargin)
...
elseif strcmp(matrices(1),'prod')
    ...
    % initial matrix
    A = DTQP_tmatrix(matrices{1},auxdata,varargin{:});

    % compute the matrix products
    for k = 2:length(matrices)
        B = DTQP_tmatrix(matrices{k},auxdata,varargin{:});
    ...
    A = DTQP_tmatrix(matrices,auxdata,varargin{:});
