...
%--------------------------------------------------------------------------
function [O,sol] = BrysonHo248_output(T,U,Y,P,F,p,opts)

[t1,t2] = BrysonHo248_T(p.alpha,p.beta,p.gamma,p.tf);

% solution on T
sol(1).T = T;
sol(1).U = BrysonHo248_U(p.alpha,p.beta,p.gamma,T,t1,t2,p.tf);
sol(1).Y = BrysonHo248_Y(p.alpha,p.beta,p.gamma,T,t1,t2,p.tf);
