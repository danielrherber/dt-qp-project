...
% solution on T
sol(1).T = T;
sol(1).U = BrysonHo153_U(auxdata.c,in.t0,in.tf,auxdata.x0)*ones(size(T));
sol(1).Y = BrysonHo153_Y(auxdata.c,T,in.t0,in.tf,auxdata.x0);
sol(1).F = BrysonHo153_F(auxdata.c,in.t0,in.tf,auxdata.x0);

% solution on high resolution T
if opts.general.plotflag
    sol(2).T = linspace(in.t0,in.tf,1e4)';
    sol(2).U = BrysonHo153_U(auxdata.c,in.t0,in.tf,auxdata.x0)*ones(size(sol(2).T));
    sol(2).Y = BrysonHo153_Y(auxdata.c,sol(2).T,in.t0,in.tf,auxdata.x0);
    sol(2).F = sol(1).F;
end
