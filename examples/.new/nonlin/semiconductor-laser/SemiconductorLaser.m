...
% Mayer term
M(1).left = 0; M(1).right = 3; M(1).matrix = 1; % p
setup.lq.mayer = M;
...
setup.nonlin.data.symbols = 'taup ep P0 B_ Imin Gp Ntr q C_ gam beta_ A_ S0 N0 ts';
setup.nonlin.data.values = [taup ep P0 B Imin Gp Ntr q C gam beta A S0 N0 ts];
...
% outputs
[O,sol] = SemiconductorLaser_output(T,U,Y,P,F,in,opts);

% plots
SemiconductorLaser_plot(T,U,Y,P,F,in,opts,sol)
