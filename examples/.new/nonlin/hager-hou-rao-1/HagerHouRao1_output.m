%--------------------------------------------------------------------------
% HagerHouRao1_output.m
% Output function for HagerHouRao1 example
%--------------------------------------------------------------------------
%
%--------------------------------------------------------------------------
% Link: https://github.com/danielrherber/dt-qp-project
%--------------------------------------------------------------------------
function [O,sol] = HagerHouRao1_output(T,U,Y,P,F,in,opts)

% solution on T
sol(1).T = T;
sol(1).U = HagerHouRao1_U(T);
sol(1).Y = HagerHouRao1_Y(T);
sol(1).F = HagerHouRao1_F;

% solution on high resolution T
if opts.general.plotflag
    sol(2).T = linspace(in.t0,in.tf,1e4)';
    sol(2).U = HagerHouRao1_U(sol(2).T);
    sol(2).Y = HagerHouRao1_Y(sol(2).T);
    sol(2).F = sol(1).F;
end
