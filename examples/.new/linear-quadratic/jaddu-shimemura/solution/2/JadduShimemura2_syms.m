...
sol_1 = dsolve( eq1, eq2, ...
    Y1(0) == 0, Y2(0) == -1, ...
    Y1(t1) == 8*(t1-0.5)^2 - 0.5, Y2(t1) == y2_t1);
...
% X = patternsearch(@(x) JadduShimemura2_objective(x),X0,A,b,[],[],LB,UB,...
%     @(x) JadduShimemura2_constraints(x),options) 

X0 = X;
options = optimoptions('fmincon','Display','Iter','UseParallel',true,...
    'Algorithm','interior-point');
X = fmincon(@(x) JadduShimemura2_objective(x),X0,A,b,[],[],LB,UB,...
    @(x) JadduShimemura2_constraints(x),options)
...
aP = matlabFunction(sym([t1,y2_t1]),'file',[path,prob,'_parameters'],'Optimize',true);

T1 = linspace(0,t1,1e5)';
T2 = linspace(t1,1,1e5)';

Y_1_T1 = JadduShimemura2_Y_t1(T1,t1,y2_t1);
Y_2_T2 = JadduShimemura2_Y_t2(T2,t1,y2_t1);
